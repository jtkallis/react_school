...
export default async function newTeacher() {
    ...
    <>
      <header className="flex justify-between items-center mb-4">
        <h1 className="text-2xl">Student</h1>
      </header>
      <form action={createStudent} className="flex gap-2 flex-col">
        <input 
            type="text"
            name="firstName"
        ...
        <select name="department"  className="border border-slate-300  bg-transparent  text-slate-300 px-2 py-1 rounded hover:bg-slate-700 focus-within:bg-slate-700 outline-none">
            {departments.map(department => (
                <option key={department.id} value={department.id}  className="border border-slate-300 text-slate-300 px-2 py-1 rounded hover:bg-slate-700 focus-within:bg-slate-700 outline-none">{department.name}</option>
            ))}
        </select>
